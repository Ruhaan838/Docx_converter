...
class Document_File_text():
    def __init__(self,code,list2,image_list=[],practical_num=1,crr_page=1,code_sub="",
                 enrollment_num="",sub_name="",sub_name_small="",file_location=""):
    ...
    def docx_file(self):
        ...
        for page_no, code in enumerate(self.code[:prac_num]):
            ...
            image_path = self.image_list[page_no]

            if path.exists(image_path):
                doc.add_picture(image_path, width=Inches(7), height=Inches(3))
            else:
                messagebox.showerror("Error", f"Error: Image file not found at {image_path}")

            if page_no < min(prac_num, len(self.image_list)) - 1:
                doc.add_page_break()
